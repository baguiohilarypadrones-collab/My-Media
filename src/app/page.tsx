...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (localItems) {
      try {
        setAllMedia(JSON.parse(localItems));
      } catch {
  ...
  const patchProgressValue = (id: string, updates: Partial<MediaProgress>) => {
    setAllMedia((prev) => 
      prev.map((item) => {
        if (item._id === id) {
        ...
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-4 px-4 py-4 sm:px-6 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs font-bold uppercase tracking-[0.25em] text-indigo-500">Offline Workspace</p>
            </div>
            ...
            <p className={cn("text-xs", darkMode ? "text-slate-400" : "text-slate-500")}>
              Your local database dashboard — zero servers required.
            </p>
          </div>
